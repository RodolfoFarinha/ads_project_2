...
namespace Api.Service.Business
{
    ...
    public static class ScheduleCalculater
    {
        ...
        /// </summary>
        public static List<SessionViewModel> CreateSessionSlots(QualityScheduleViewModel qualitySchedule, List<SessionViewModel> sessions, List<RoomViewModel> rooms)
        {
            ...
            {
                RoomViewModel generatedRoom = GetAvailableRoom(rooms, session);
        ...
        /// </summary>
        public static RoomViewModel GetAvailableRoom(IEnumerable<RoomViewModel> rooms, SessionViewModel session)
        {
            foreach (RoomViewModel room in rooms)
                if (!room.Slots.Where(x => x.StartDate > session.StartDate && x.EndDate < session.EndDate).Any() &&
                    ...
                    room.NormalCapacity >= session.Shift.EnrolledStudents)
                    return room;
